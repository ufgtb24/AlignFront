...
class Graph:
    ...
    def build(self,
        ...
        decoder_outputs = self.build_decoder(decoder_emb_inp, self.encoder_outputs)#[batch_size, max_sequence_len,dim]
        output =  self.build_output(decoder_outputs)

        if self.mode == tf.estimator.ModeKeys.TRAIN:
            # 训练只需要一步，而不需要循环是因为不需要因果递推产生输入，在token之间实现了并行
    ...
    def _filled_next_token(self, inputs, logits, decoder_index):
        tf.identity(tf.argmax(logits[0], axis=1, output_type=tf.int32), 'test/pred_{decoder_index}')
